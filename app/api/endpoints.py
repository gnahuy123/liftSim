...
from app.models.schemas import (
    PassengerRequest, CreateSessionRequest, CreateComparisonRequest
)
...
async def create_session(request: CreateSessionRequest = None):
    ...
    algorithm_name = request.algorithm if request else "scan"
...
async def create_comparison(request: CreateComparisonRequest = None):
    """Create a comparison session with 2 buildings, each with 2 lifts."""
    algo1 = request.algorithm1 if request else "scan"
    algo2 = request.algorithm2 if request else "scan"
...
async def get_state(session_id: str):
    ...
    try:
        ...
        if session_type == "comparison":
            ...
            return {
                "type": "single",
                **transform_building_state(state)
            }
